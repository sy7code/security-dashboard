...
import { Shield, Plus, Edit2, Trash2, Activity, Clock, CheckCircle2, XCircle } from "lucide-react";
          ...
          <div key={scanner.id} className="bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 p-6 rounded-2xl relative group hover:border-indigo-500/30 transition-all duration-300">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between text-sm border-t border-slate-700/50 pt-3 mt-3">
                ...
                <span className="text-slate-200 font-mono bg-slate-900/50 px-2 py-0.5 rounded text-xs select-all">{scanner.apiKey}</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                  placeholder="예: Snyk CLI"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1">빈도 (일)</label>
                  <input
                    type="number"
                    min="1"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={editingScanner ? "기존 키를 유지하려면 비워두세요" : "API 키를 입력하세요"}
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all font-mono text-sm"
                />
                {editingScanner && <p className="text-xs text-slate-500 mt-1">이 부분을 ********로 남겨두어도 현재 키는 변경되지 않습니다.</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">API URL (선택 사항)</label>
                <input
                  type="url"
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-800">
                ...
                <button
                  type="submit"
                  className="flex-1 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-all shadow-lg hover:shadow-indigo-500/25"
                >
                  {editingScanner ? "저장" : "추가"}
                </button>
              </div>
            </form>
          </div>
